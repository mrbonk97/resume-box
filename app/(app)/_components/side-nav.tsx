...
export const Sidenav: React.FC<SidenavProps> = ({ handleScroll, refs }) => {
  ...
  return (
    <aside className="fixed top-0 left-20 p-5 pt-28 h-full w-96 flex flex-col items-center bg-secondary/50 border-r">
      ...
      <ul className="w-full space-y-5 p-5">
        {MENU.map((item) => (
          <li
            key={item.idx}
            className={`p-2 menu-item hover:bg-rose-100 cursor-pointer text-right ${
